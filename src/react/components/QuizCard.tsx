    ...
    return <Card role='button' className='border-2 rounded-lg border-[#777] p-4 cursor-pointer bg-[rgba(0,0,0,0.2)]' onClick={() => navigateToQuiz(quiz.quizId)}>
        <div className="flex">
            <div className="flex-1 flex flex-col items-start">
                <p className='flex gap-2 items-center justify-center'>{quiz.maxScore} points ({quiz.type} quiz) {quiz.isOpen ? <FaLockOpen size={13} color="lime" /> : <FaLock size={13} color='yellow' />}</p>
